<?php
 ...
 if (isset($_SESSION['auth'])) {
     ...
     $sql = "select record.Request, record.PersonalCode, personal.Fio, post.PostValue, record.Data, record.Time, record.Contact
     from (((record 
     ...
     WHERE `UserCode`='$idUs' and record.Request = 'Закрыта' ORDER BY `Data`, `Time` ASC ";
     if($result = $link->query($sql)){
         foreach($result as $row){
             echo '<div class="block__record__content__close">';
     ...
     $sql = ("select record.Data, record.Time, users.fio, record.Contact
     from (record 
     inner join users on record.UserCode = users.UsersId)
     WHERE `PersonalCode`='$idUsDoc' and record.Request = 'Закрыта' ORDER BY `Data`, `Time` ASC");
     if($result = $link->query($sql)){
         foreach($result as $row){
             echo '<div class="block__record__content__close">';
             echo '<div class="evCont__record__post">' . $row["fio"] . '</div>';
